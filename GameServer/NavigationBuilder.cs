...
namespace GameServer
{
    class NavigationBuilder
    {
        ...
        static Dictionary<string, Vector3[]> verticesList;
        static Dictionary<string, int[]> indicesList;
        static Dictionary<string,List<PointGraph>> graphList;

        public static void Initialize()
        {
            verticesList = new Dictionary<string, Vector3[]>();
            indicesList = new Dictionary<string, int[]>();
            graphList = new Dictionary<string, List<PointGraph>>();
        }

        public static void AddMap(string mapName, Vector3[] vert, int[] inds, List<PointGraph> pts)
        {
            verticesList.Add(mapName, vert);
            indicesList.Add(mapName, inds);
            graphList.Add(mapName, pts);
        }

        public static List<Vector3> GetShortestPath(string mapName, Vector3 point1, Vector3 point2)//, out Vector3[] vmesh, out int[] imesh)
        {

            var vertices = verticesList[mapName];
            var indices = indicesList[mapName];
            var pointGraphs = graphList[mapName];
            ...
            while (openList.Count != 0)
            {
                ...
                if (Vector3.Distance(p2proj, vertices[n.index])<Vector3.Distance(p2proj, vertices[closest.index])) closest = n;
            ...
            // On ajoute le dernier noeud que si on l'atteint, sinon on se rapproche au max.
            if (found)
                path.Add(p2proj);
        ...
        public static int NearestTriangle(string mapName, Vector3 point, out Vector3 actualProjection)
        {
            var vertices = verticesList[mapName];
            var indices = indicesList[mapName];
            return NearestTriangle(vertices, indices, point , out actualProjection);
        }
        ...
        static int NearestTriangle(Vector3[] vertices, int[] indices, Vector3 point, out Vector3 actualProjection)
        {
            ...
            {
                float distance = 0;
                distance = (ClosestPointOnTriangle(vertices[indices[i * 3 + 0]], vertices[indices[i * 3 + 1]], vertices[indices[i * 3 + 2]], point) - point).LengthSquared(); // LenghtSquared saves a squareroot each time;
